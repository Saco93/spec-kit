...
def get_github_auth_token() -> Optional[str]:
    """Try to get GitHub authentication token using GitHub CLI (gh).

    Returns:
        The auth token if gh CLI is installed and authenticated, None otherwise.
    """
    if not shutil.which("gh"):
        return None

    try:
        # Try to get the auth token using gh CLI
        ...
        # Silently ignore any other errors - we'll fall back to anonymous requests
        pass
...
def download_template_from_github(
    ...
    *,
    ...
    try:
        # Try to get GitHub CLI authentication token to avoid rate limits
        github_token = get_github_auth_token()
        headers = {}
        if github_token:
            headers["Authorization"] = f"token {github_token}"
            if verbose:
                console.print("[green]✓[/green] Using GitHub CLI authentication")

        response = client.get(
            api_url, timeout=30, follow_redirects=True, headers=headers
        )
...
def init(
    ...
    ai_assistant: str = typer.Option(
        None,
        "--ai",
        help="AI assistant to use: claude, gemini, copilot, cursor, qwen or opencode",
    ),
